(function (global, factory) {
    ...
    class Lightbox {
        ...
        open(src, triggerEl) {
            ...
            console.log('[lb3] open path:', { thumbSrc, src, fullResReady, overlaySrc, willSwap: !!(thumbSrc && thumbSrc !== src) });
...
// Skip when the full-res is already loaded — createOverlay assigned the correct src,
// and re-assigning it mid-FLIP causes WebKit to cache a low-res raster sized to the
// scaled-down composited layer.
if (thumbSrc && thumbSrc !== src && !fullResReady) {
    ...
    this.animateSpring({ translateX: flipX, translateY: flipY, scale: flipScale, opacity: 0, crop: hasCrop ? 1 : 0, borderRadius: this.thumbBorderRadius }, { translateX: 0, translateY: 0, scale: 1, opacity: 1, crop: 0, borderRadius: this.getTargetBorderRadius() }, this.opts.springOpen, () => {
        ...
        this.emit('opened');
        // Replace the img element entirely — guarantees a fresh GPU layer
    // and a fresh raster decode at the final layout size.
    if (this.imgEl && this.currentSlideEl) {
        const oldImg = this.imgEl;
        const newImg = document.createElement('img');
        newImg.className = 'lightbox3-image';
        newImg.alt = '';
        newImg.draggable = false;
        // Copy positioning styles
        newImg.style.left = oldImg.style.left;
        newImg.style.top = oldImg.style.top;
        newImg.style.width = oldImg.style.width;
        newImg.style.height = oldImg.style.height;
        newImg.style.borderRadius = oldImg.style.borderRadius;
        // Re-bind handlers
        newImg.addEventListener('click', (e) => this.handleImageClick(e));
        newImg.addEventListener('pointerdown', this.handleImagePointerDown);
        newImg.addEventListener('pointermove', this.handlePointerMove);
        newImg.addEventListener('pointerup', this.handlePointerUp);
        newImg.addEventListener('pointercancel', this.handlePointerUp);
        // Swap in DOM
        newImg.src = oldImg.src;
        this.currentSlideEl.replaceChild(newImg, oldImg);
        this.imgEl = newImg;
    }
    }, undefined, undefined, undefined, openVisuallyDone);
};

if (this.imgEl && this.imgEl.src && typeof this.imgEl.decode === 'function') {
    // Force WebKit to decode at the element's full layout size BEFORE
    // we apply any FLIP-scaled transform. Without this, WebKit caches
    // a low-res raster sized to the composited (scaled-down) layer.
    this.imgEl.decode().then(startOpenSpring).catch(startOpenSpring);
} else {
    startOpenSpring();
}
...
swapToFullRes(src) {
    console.log('[lb3] swapToFullRes called for', src);
    ...
    this.loadImage(src).then((size) => {
        ...
        // ─── WebKitGTK full-resolution decode fix ──────────────────────
        // WebKit picks raster size from the composited (post-transform)
        // layer size when the image decodes. During the FLIP open
        // animation the transform scales the layer down to thumbnail
        // size, so a naive `el.src = src` causes WebKit to cache a
        // low-res bitmap that persists when the spring restores scale(1).
        // Pause the open spring, neutralize the transform, assign src,
        // force layout, await decode, then snap the animation to its
        // resting state so the rest of the open completes cleanly.
        const hadActiveSpring = this.rafId !== null;
        if (hadActiveSpring) {
        ...
        el.src = src;
        // Force layout so WebKit picks up the new (full) size before
        // dispatching decode work.
        void el.offsetHeight;

        const finishSwap = () => {
            if (!this.imgEl || this.imgEl !== el)
                return;
            if (this.state.isClosing || !this.state.isOpen)
                return;

            // If we cut off the open spring, snap to its end state so the
            // animation doesn't freeze mid-FLIP.
            if (hadActiveSpring) {
                el.style.transform = '';
                if (this.backdrop)
                    this.backdrop.style.opacity = '1';
                this.chromeBaseOpacity = 1;
                this.chromeDriftProgress = 0;
            ...
            // Fade in on full-res swap — only when the swap was not
            // triggered by a navigation. During navigation the strip
        ...
        // Use decode() to wait until the bitmap is rasterized at the
        // (now full-size) layout before we restore the transform.
        if (typeof el.decode === 'function') {
            el.decode().then(finishSwap).catch(finishSwap);
        } else {
            finishSwap();
        }
    });
